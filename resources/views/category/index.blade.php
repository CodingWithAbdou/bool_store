                    ...
                    <thead class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            <th  class="px-6 py-3 rounded-lg">
                                <h2 class="text-xl">عرض التصنيفات</h2>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr class="bg-white dark:bg-gray-800 ">
                            <td class="px-6 py-4">
                                <div>
                                    <div class="mb-3">
                                        <form  class="relative mb-4 flex mx-auto flex-wrap items-stretch w-[80%]" action="{{route('category.search')}}"   method="GET" >
                        ...
                        @if($categories->count() > 0 )
                            @foreach ($categories as $category)
                                <tr class="bg-white dark:bg-gray-800  border rounded-lg">
                                    ...
                                    <td class="px-6 py-4">
                                        <a class="hover:text-blue-600"  href="{{ route('category.show' , $category) }}" >
                                            {{ $category->name  }}
