                    ...
                    <tbody>
                        <tr class="bg-white dark:bg-gray-800 ">
                            <td class="px-6 py-4">
                                <div>
                                    <div class="mb-3">
                                        <form  class="relative mb-4 flex mx-auto flex-wrap items-stretch w-[80%]"  action="{{ route('author.search') }}"   method="GET" >
                        ...
                        @if($authors->count() > 0 )
                            @foreach ($authors as $author)
                                <tr class="bg-white dark:bg-gray-800  border rounded-lg">
                                    <td class="py-4 px-6">
                                        <a class="hover:text-blue-600"  href="{{ route('author.list' , $author) }}" >
                                            {{ $author->name  }}
                                            ({{ $author->books->count()}})
                                        </a>
                                    </td>
